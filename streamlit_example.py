...
# --------------------------------------------------
# 4) 지표 선택 버튼 – 왼쪽 정렬로 ‘다닥다닥’
# --------------------------------------------------
left, mid, right = st.columns([0.18, 0.18, 0.64])   # 여백 없이 왼쪽 두 칸에만 버튼
with left:
    if st.button(metrics["completion_rate"]):
        st.session_state.selected_metric = "completion_rate"
with mid:
    if st.button(metrics["re_subscription_rate"]):
        st.session_state.selected_metric = "re_subscription_rate"
with right:
    if st.button(metrics["ad_click_rate"]):
...
fig.update_traces(
    hovertemplate=f"<b>%{{location}}</b><br>{metric_kor} = %{{z:.2f}}%<extra></extra>"
)
fig.update_layout(
...
if selected_state:
    ...
    fig.add_trace(
        go.Choropleth(
            ...
            marker_line_color="#c40028", marker_line_width=1.5
        )
    )
...
if selected_state:
    ...
    trend_fig.update_traces(
        hovertemplate=f"%{{x}}<br>{metric_kor} = %{{y:.2f}}%<extra></extra>"
    )
